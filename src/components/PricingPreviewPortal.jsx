...
const PricingPreviewPortal = ({ position, onSelect, onClose }) => {
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      >
        <div className="grid grid-cols-2 gap-x-8 gap-y-12 px-8 pt-12"> {/* 2-column grid with proper spacing */}
          {previewBlocks.map(({ id, component: Component }) => (
            <div
              key={id}
              className="rounded-lg overflow-hidden hover:shadow-md transition-shadow cursor-pointer bg-gray-50" // Added rounded + hover for consistency & better look
              onClick={() => {
                onSelect(id);
                onClose();
              }}
...
//         }}
//       >
//         <div className="grid grid-cols-2 gap-x-8 gap-y-12 px-8 pt-12">
//           {previewBlocks.map(({ id, component: Component }) => (
//             <div
//               key={id}
//               className="rounded-lg overflow-hidden hover:shadow-md transition-shadow cursor-pointer bg-gray-50 flex justify-center items-center"
//               onClick={() => {
//                 onSelect(id);
//                 onClose();
//               }}
//               style={{
//                 height: "240px", // Increased height for better centering + breathing room
//               }}
//             >
//               <div
//                 style={{
//                   transform: "scale(0.35)", // Slightly larger scale to fill the box better while keeping centering
