...
const PricingAndServices = ({ blockId, parentId, isPreview }) => {
  ...
  const calculateItemTotals = (item) => {
    ...
    if (item.vatType === "exclusive" && vatRate > 0) {
      vatAmount = total * (vatRate / 100);
      total += vatAmount;
    } else if (item.vatType === "inclusive" && vatRate > 0) {
      vatAmount = total - total / (1 + vatRate / 100);
      total = subtotal - rutDiscountAmount - rotDiscountAmount;
    } else if (item.vatType === "reverse") {
      vatAmount = total * (vatRate / 100);
    }

    // Apply Environment Tax (assumed exclusive on final total after discounts and VAT)
    if (envTaxRate > 0) {
    ...
    return {
      ...
      vatAmount,
      envTaxAmount,
      total: item.vatType === "reverse" ? total : total + (item.vatType === "exclusive" ? vatAmount : 0),
    };
  };
