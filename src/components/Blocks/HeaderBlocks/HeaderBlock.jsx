...
//         const url = exists
//           ? `${API_URL}/api/header/${id}` // update
//           : `${API_URL}/api/CreateHeaderBlock`; // create

//         const method = exists ? "POST" : "POST";
...
//       const url = isCreated
//         ? `${API_URL}/api/header/${headerId}`
//         : `${API_URL}/api/CreateHeaderBlock`;

//       const method = isCreated ? "POST" : "POST";
...
const HeaderBlock = ({
  ...
  console.log("i am parentId from header block", parentId);
  console.log("HeaderBlock - isExisting:", isExisting, "id:", id, "headerId:", headerId);
  ...
  useEffect(() => {
    if (isExisting && !isCreated) {
      setIsCreated(true);
      setHeaderId(id);
      console.log("Setting isCreated to true because isExisting is true");
    }
  ...
  const saveHeaderBlock = useCallback(async () => {
    ...
    try {
        ...
        try {
          ...
          if (checkRes.ok) {
            ...
            if (result && result.id) {
              ...
              finalId = result.id;
              if (!headerId) {
                setHeaderId(result.id);
              }
              if (!isCreated) {
                setIsCreated(true);
              }
              console.log("Found existing header with ID:", result.id);
            }
          }
        } catch (error) {
          console.log("Error checking for existing header:", error);
        }
      }
      ...
      console.log("Saving header - shouldUpdate:", shouldUpdate, "finalId:", finalId, "url:", url);
      ...
      if (!response.ok) {
        ...
        if (!shouldUpdate && result?.data?.id) {
          ...
          console.log("HeaderId saved:", newHeaderId);
  ...
  }, [headerBlock, id, parentId, userId, isPreview, headerId, isCreated, dispatch]);
  ...
  const handleCropSave = async () => {
    try {
      ...
      console.log("Saving logo - shouldUpdate:", shouldUpdate, "finalId:", finalId);
      ...
      const saveData = await saveRes.json();
      console.log("Logo save response:", saveData);
      ...
      if (!saveRes.ok) {
        ...
        if (!shouldUpdate && saveData?.data?.id) {
          ...
          console.log("Created new header with ID:", newHeaderId);
        }
      }
    } catch (err) {
      console.error("Error uploading and saving logo:", err);
