...
const HeaderBlock2 = ({
  ...
  textAlign,
  isPreview = false,
  isExisting = false, // Add this prop to indicate edit mode
  layoutStyles = null,
}) => {
  const userId = useSelector(selectedUserId);
  const [isCreated, setIsCreated] = useState(isExisting); // Initialize with isExisting
  const [headerBlock, setHeaderBlock] = useState({
    layoutType,
    title: initialTitle,
    subtitle: initialSubtitle,
    ...
    senderName: initialSenderName,
    price: initialPrice,
    logoUrl: initialLogo, // Use initialLogo prop
    backgroundImage: backgroundImage,
    backgroundColor: backgroundColor,
    textColor: textColor || "#ffffff",
    backgroundFilter: backgroundFilter || null,
  ...
  console.log("HeaderBlock2 - isExisting:", isExisting, "id:", id, "headerId:", headerId, "parentId:", parentId);
  ...
  useEffect(() => {
    setHeaderBlock((prev) => ({
      ...
      textAlign: textAlign || prev.textAlign,
      layoutStyles: layoutStyles || LAYOUTS[layoutType] || LAYOUTS.default,
      logoUrl: initialLogo || prev.logoUrl, // Update logoUrl from initialLogo prop
    }));
  }, [
    layoutType,
    backgroundImage,
    ...
    textAlign,
    layoutStyles,
    initialLogo, // Add initialLogo to dependencies
  ]);

  // Initialize isCreated state based on isExisting prop
  useEffect(() => {
    if (isExisting && !isCreated) {
      setIsCreated(true);
      setHeaderId(id);
      console.log("HeaderBlock2: Setting isCreated to true because isExisting is true");
    }
  }, [isExisting, id, isCreated]);
  ...
  const checkHeaderExists = useCallback(async () => {
    if (!id || !parentId) return false;

    try {
      // Check if there's already a header block for this parent AND block ID
      const checkRes = await fetch(
        `${API_URL}/api/headerBlock?parentId=${parentId}&blockId=${id}`
      );

      if (checkRes.ok) {
        const result = await checkRes.json();
        console.log("HeaderBlock2 check result:", result);

        if (result && result.id) {
          // We found an existing header for this block
          return { exists: true, headerId: result.id };
        }
      }
    } catch (error) {
      console.log("HeaderBlock2: Error checking for existing header:", error);
    }

    return { exists: false, headerId: null };
  }, [id, parentId]);

  // Save header block function
  const saveHeaderBlock = useCallback(async () => {
    if (isPreview) return;

    try {
      // If we're in edit mode (isExisting) and have a headerId, we should update
      // Otherwise, we need to check if this header already exists
      let shouldUpdate = isCreated && headerId;
      let finalId = headerId || id;

      // If we don't know yet, check the server
      if (!shouldUpdate && parentId && id) {
        try {
          const { exists, headerId: existingId } = await checkHeaderExists();
          if (exists && existingId) {
            shouldUpdate = true;
            finalId = existingId;
            if (!headerId) {
              setHeaderId(existingId);
            }
            if (!isCreated) {
              setIsCreated(true);
            }
            console.log("HeaderBlock2: Found existing header with ID:", existingId);
          }
        } catch (error) {
          console.log("HeaderBlock2: Error checking for existing header:", error);
        }
      ...
      const url = shouldUpdate
        ? `${API_URL}/api/header/${finalId}` // update
        : `${API_URL}/api/CreateHeaderBlock`; // create

      const method = shouldUpdate ? "POST" : "POST";

      console.log("HeaderBlock2 Saving - shouldUpdate:", shouldUpdate, "finalId:", finalId, "url:", url);
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          userId: userId,
          layoutStyles: headerBlock.layoutStyles,
          blockId: id, // Include the block ID for reference
        }),
      });

      if (!response.ok) {
        const errorData = await response.json();
        console.error("HeaderBlock2: Failed to save header block:", errorData);
      } else {
        const result = await response.json();
        console.log("HeaderBlock2: Header block saved successfully:", result);

        // ✅ If newly created, store ID and mark as created
        if (!shouldUpdate && result?.data?.id) {
          setIsCreated(true);
          const newHeaderId = result.data.id;
          setHeaderId(newHeaderId);
          dispatch(addHeaderId(newHeaderId));
          console.log("HeaderBlock2: HeaderId saved:", newHeaderId);
        } else if (shouldUpdate && !headerId && finalId) {
          // Ensure we have the headerId in state for updates
          setHeaderId(finalId);
          setIsCreated(true);
        }
      }
    } catch (error) {
      console.error("HeaderBlock2: Error saving header block:", error);
    }
  }, [headerBlock, id, parentId, userId, isPreview, headerId, isCreated, dispatch, checkHeaderExists]);

  // Use effect to save header block with debouncing
  useEffect(() => {
    if (isPreview) return;
    ...
    return () => clearTimeout(timeout);
  }, [headerBlock, isPreview, saveHeaderBlock]);
  ...
  const handleLogoReset = () => {
    ...
    updateHeaderBlock({ logoUrl: null });
    setImageSrc(null);
    setShowLogo(true);
    setCropModalOpen(false);
  ...
  const handleCropSave = async () => {
    try {
      ...
      if (!uploadData.success) {
        console.error("HeaderBlock2: Upload failed:", uploadData.message);
        return;
      }
      ...
      // If we're not sure, check the server
      if (!shouldUpdate && parentId && id) {
        try {
          const { exists, headerId: existingId } = await checkHeaderExists();
          if (exists && existingId) {
      ...
      const url = shouldUpdate
        ? `${API_URL}/api/header/${finalId}`
        : `${API_URL}/api/CreateHeaderBlock`;

      const method = shouldUpdate ? "POST" : "POST";

      console.log("HeaderBlock2 Saving logo - shouldUpdate:", shouldUpdate, "finalId:", finalId);
      ...
      const saveData = await saveRes.json();
      console.log("HeaderBlock2 Logo save response:", saveData);

      if (!saveRes.ok) {
        console.error("HeaderBlock2: Failed to save header block with new logo:", saveData);
      } else {
        console.log("HeaderBlock2: Header block saved with updated logo:", saveData);

        updateHeaderBlock({ logoUrl: updatedLogoUrl });
        setCropModalOpen(false);

        // Update local ID if newly created
        if (!shouldUpdate && saveData?.data?.id) {
          setIsCreated(true);
          const newHeaderId = saveData.data.id;
          setHeaderId(newHeaderId);
          localStorage.setItem("headerId", newHeaderId);
          console.log("HeaderBlock2: Created new header with ID:", newHeaderId);
        }
      }
    } catch (err) {
      console.error("HeaderBlock2: Error uploading and saving logo:", err);
    }
  };
  ...
  const getAdjustedColumns = () => {
    ...
    // Use flexbox for resizable layouts instead of Bootstrap grid
    const imageCol = `flex-shrink-0 ${headerBlock.layoutType === "left-panel" ? "order-1" : "order-2"
      }`;
    const contentCol = `flex-grow-1 ${headerBlock.layoutType === "left-panel" ? "order-2" : "order-1"
      }`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content Section */}
        <div
          className={`${contentCol} ${headerBlock.layoutType === "grid"
              ? ""
              : "d-flex flex-column justify-content-between"
            }`}
          style={{
        ...
        >
          ...
          {headerBlock.layoutType !== "default" && (
            ...
            >
              ...
              >
                <div
                  className={`d-inline-flex align-items-center ${(headerBlock.clientName || "").trim() === ""
                      ? "rounded-pill"
                      : ""
                    }`}
                  style={{
              ...
              >
                <div
                  className={`d-inline-flex align-items-center ${(headerBlock.senderName || "").trim() === ""
                      ? "rounded-pill"
                      : ""
                    }`}
                  style={{
