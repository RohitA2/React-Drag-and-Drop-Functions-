...
const HeaderBlock5 = ({
  ...
  backgroundImage = `${API_URL}/uploads/1766832158465.avif`,
  backgroundColor = "#B99698",
  textColor,
  backgroundFilter,
  textAlign,
  ...
  console.log("HeaderBlock5 - isExisting:", isExisting, "id:", id, "headerId:", headerId, "parentId:", parentId);
  ...
  useEffect(() => {
    if (isExisting && !isCreated) {
      setIsCreated(true);
      setHeaderId(id);
      console.log("HeaderBlock5: Setting isCreated to true because isExisting is true");
    }
  ...
  const checkHeaderExists = useCallback(async () => {
    ...
    try {
      ...
      if (checkRes.ok) {
        ...
        console.log("HeaderBlock5 check result:", result);
      ...
      console.log("HeaderBlock5: Error checking for existing header:", error);
    }
  ...
  const saveHeaderBlock = useCallback(async () => {
    ...
    try {
      ...
      if (!shouldUpdate && parentId && id) {
        try {
          ...
          if (exists && existingId) {
            ...
            console.log("HeaderBlock5: Found existing header with ID:", existingId);
          }
        } catch (error) {
          console.log("HeaderBlock5: Error checking for existing header:", error);
        }
      ...
      console.log("HeaderBlock5 Saving - shouldUpdate:", shouldUpdate, "finalId:", finalId, "url:", url);
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error("HeaderBlock5: Failed to save header block:", errorData);
      } else {
        const result = await response.json();
        console.log("HeaderBlock5: Header block saved successfully:", result);
        ...
        if (!shouldUpdate && result?.data?.id) {
          ...
          console.log("HeaderBlock5: HeaderId saved:", newHeaderId);
      ...
      console.error("HeaderBlock5: Error saving header block:", error);
    }
  ...
  const handleCropSave = async () => {
    try {
      ...
      if (!uploadData.success) {
        console.error("HeaderBlock5: Upload failed:", uploadData.message);
        return;
      }
      ...
      console.log("HeaderBlock5 Saving logo - shouldUpdate:", shouldUpdate, "finalId:", finalId);
      ...
      const saveData = await saveRes.json();
      console.log("HeaderBlock5 Logo save response:", saveData);

      if (!saveRes.ok) {
        console.error("HeaderBlock5: Failed to save header block with new logo:", saveData);
      } else {
        console.log("HeaderBlock5: Header block saved with updated logo:", saveData);
        ...
        if (!shouldUpdate && saveData?.data?.id) {
          ...
          console.log("HeaderBlock5: Created new header with ID:", newHeaderId);
        }
      }
    } catch (err) {
      console.error("HeaderBlock5: Error uploading and saving logo:", err);
    }
  };
