...
const HeaderBlock4 = ({
  ...
  console.log("HeaderBlock4 - isExisting:", isExisting, "id:", id, "headerId:", headerId, "parentId:", parentId);
  ...
  useEffect(() => {
    if (isExisting && !isCreated) {
      setIsCreated(true);
      setHeaderId(id);
      console.log("HeaderBlock4: Setting isCreated to true because isExisting is true");
    }
  ...
  const checkHeaderExists = useCallback(async () => {
    ...
    try {
      ...
      if (checkRes.ok) {
        ...
        console.log("HeaderBlock4 check result:", result);
      ...
      console.log("HeaderBlock4: Error checking for existing header:", error);
    }
  ...
  const saveHeaderBlock = useCallback(async () => {
    ...
    try {
      ...
      if (!shouldUpdate && parentId && id) {
        try {
          ...
          if (exists && existingId) {
            ...
            console.log("HeaderBlock4: Found existing header with ID:", existingId);
          }
        } catch (error) {
          console.log("HeaderBlock4: Error checking for existing header:", error);
        }
      ...
      console.log("HeaderBlock4 Saving - shouldUpdate:", shouldUpdate, "finalId:", finalId, "url:", url);
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error("HeaderBlock4: Failed to save header block:", errorData);
      } else {
        const result = await response.json();
        console.log("HeaderBlock4: Header block saved successfully:", result);
        ...
        if (!shouldUpdate && result?.data?.id) {
          ...
          console.log("HeaderBlock4: HeaderId saved:", newHeaderId);
      ...
      console.error("HeaderBlock4: Error saving header block:", error);
    }
  ...
  const handleCropSave = async () => {
    try {
      ...
      if (!uploadData.success) {
        console.error("HeaderBlock4: Upload failed:", uploadData.message);
        return;
      }
      ...
      console.log("HeaderBlock4 Saving logo - shouldUpdate:", shouldUpdate, "finalId:", finalId);
      ...
      const saveData = await saveRes.json();
      console.log("HeaderBlock4 Logo save response:", saveData);

      if (!saveRes.ok) {
        console.error("HeaderBlock4: Failed to save header block with new logo:", saveData);
      } else {
        console.log("HeaderBlock4: Header block saved with updated logo:", saveData);
        ...
        if (!shouldUpdate && saveData?.data?.id) {
          ...
          console.log("HeaderBlock4: Created new header with ID:", newHeaderId);
        }
      }
    } catch (err) {
      console.error("HeaderBlock4: Error uploading and saving logo:", err);
    }
  };
