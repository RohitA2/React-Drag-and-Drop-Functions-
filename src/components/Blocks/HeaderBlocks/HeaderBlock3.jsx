...
const HeaderBlock3 = ({
  ...
  console.log("HeaderBlock3 - isExisting:", isExisting, "id:", id, "headerId:", headerId, "parentId:", parentId);
  ...
  useEffect(() => {
    if (isExisting && !isCreated) {
      setIsCreated(true);
      setHeaderId(id);
      console.log("HeaderBlock3: Setting isCreated to true because isExisting is true");
    }
  ...
  const checkHeaderExists = useCallback(async () => {
    ...
    try {
      ...
      if (checkRes.ok) {
        ...
        console.log("HeaderBlock3 check result:", result);
      ...
      console.log("HeaderBlock3: Error checking for existing header:", error);
    }
  ...
  const saveHeaderBlock = useCallback(async () => {
    ...
    try {
      ...
      if (!shouldUpdate && parentId && id) {
        try {
          ...
          if (exists && existingId) {
            ...
            console.log("HeaderBlock3: Found existing header with ID:", existingId);
          }
        } catch (error) {
          console.log("HeaderBlock3: Error checking for existing header:", error);
        }
      ...
      console.log("HeaderBlock3 Saving - shouldUpdate:", shouldUpdate, "finalId:", finalId, "url:", url);
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error("HeaderBlock3: Failed to save header block:", errorData);
      } else {
        const result = await response.json();
        console.log("HeaderBlock3: Header block saved successfully:", result);
        ...
        if (!shouldUpdate && result?.data?.id) {
          ...
          console.log("HeaderBlock3: HeaderId saved:", newHeaderId);
      ...
      console.error("HeaderBlock3: Error saving header block:", error);
    }
  ...
  const handleCropSave = async () => {
    try {
      ...
      if (!uploadData.success) {
        console.error("HeaderBlock3: Upload failed:", uploadData.message);
        return;
      }
      ...
      console.log("HeaderBlock3 Saving logo - shouldUpdate:", shouldUpdate, "finalId:", finalId);
      ...
      const saveData = await saveRes.json();
      console.log("HeaderBlock3 Logo save response:", saveData);

      if (!saveRes.ok) {
        console.error("HeaderBlock3: Failed to save header block with new logo:", saveData);
      } else {
        console.log("HeaderBlock3: Header block saved with updated logo:", saveData);
        ...
        if (!shouldUpdate && saveData?.data?.id) {
          ...
          console.log("HeaderBlock3: Created new header with ID:", newHeaderId);
        }
      }
    } catch (err) {
      console.error("HeaderBlock3: Error uploading and saving logo:", err);
    }
  };
