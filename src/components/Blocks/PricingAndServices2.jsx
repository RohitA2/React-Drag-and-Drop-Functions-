...
const PackageCard = ({
  ...
  const calculateItemTotals = (item) => {
    ...
    const taxRate = parseFloat(item.tax) || globalVatRate;
    const rutDiscount = parseFloat(item.rutDiscount) || globalRutDiscount;
    const rotRate = parseFloat(item.rotRate) || globalRotRate;
    ...
    // Calculate ROT
    if (rotRate > 0) {
      rotAmount = total * (rotRate / 100);
      total += rotAmount;
    }

    // Calculate Environment Tax
    if (envTaxRate > 0) {
      envTaxAmount = total * (envTaxRate / 100);
      total += envTaxAmount;
    }

    // Calculate VAT based on type
    const vatType = item.vatType || globalVatType;
    if (vatType === "exclusive" && taxRate > 0) {
      taxAmount = total * (taxRate / 100);
      total += taxAmount;
    } else if (vatType === "inclusive" && taxRate > 0) {
      taxAmount = total - total / (1 + taxRate / 100);
      total = subtotal - rutDiscountAmount + rotAmount + envTaxAmount;
    } else if (vatType === "reverse") {
      taxAmount = total * (taxRate / 100);
    }
    ...
    return {
      ...
      total:
        vatType === "reverse"
          ? total
          : total + (vatType === "exclusive" ? taxAmount : 0),
    };
  };
  ...
  const vatTotal = useMemo(
    () => itemTotals.reduce((sum, item) => sum + item.taxAmount, 0),
    [itemTotals]
  );
  ...
  const rotTotal = useMemo(
    () => itemTotals.reduce((sum, item) => sum + item.rotAmount, 0),
    [itemTotals]
  );
  ...
  const rounding = 0;
  const total =
    netTotal + vatTotal + rotTotal + envTaxTotal + rounding - rutDiscountTotal;
            ...
            <div className="space-y-2 small">
              ...
              {rotTotal > 0 && (
                <div className="flex justify-between items-center text-warning">
                  <span>ROT ({globalRotRate}%)</span>
                  <span className="fw-semibold">
                    {currency.symbol} {rotTotal.toFixed(2)}
                  </span>
                </div>
              )}
                ...
                <div className="col-md-4">
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      step="0.1"
                      value={
                        pkg.items[currentItemIndex].rotRate || globalRotRate
                      }
                      onChange={(e) =>
                        setItem(
                          currentItemIndex,
...
const PricingAndServices2 = ({ blockId, parentId }) => {
  ...
  const [globalRotRate, setGlobalRotRate] = useState(0);
            ...
            <div className="px-2 py-3 space-y-4">
              ...
              <Form.Group>
                ...
                <Form.Control
                  type="number"
                  value={globalRotRate}
                  onChange={(e) => setGlobalRotRate(parseFloat(e.target.value))}
                  min="0"
                  max="100"
                  step="0.1"
                  size="sm"
