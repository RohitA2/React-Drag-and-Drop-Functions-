...
//       externalParentId = null,
//       forceNewProject = false,
//       isNewProject = false, // Add this prop
//     },
//     ref
//   ) => {
//     const dispatch = useDispatch();
...
//     console.log("Canvas - externalParentId:", externalParentId);
//     console.log("Canvas - forceNewProject:", forceNewProject);
//     console.log("Canvas - isNewProject:", isNewProject);
//     console.log("Canvas - parentId:", parentId);
//     console.log("Canvas - isParentReady:", isParentReady);
//     console.log("Canvas - isLoaded:", isLoaded);
//     console.log("Canvas - loadingStatus:", loadingStatus);
...
//   setLoadingStatus("creating_parent");
//   try {
//     // Safely parse company_id from localStorage to avoid sending string "null"
//     let company_id = null;
...
//         // Priority 1: If forceNewProject or isNewProject is true, always create new
//         if (forceNewProject || isNewProject) {
//           console.log("🆕 FORCED NEW PROJECT - Creating fresh canvas");
//           setIsForcedNew(true);
...
//           const newParentId = await createParent();
//           if (newParentId) {
//             // Don't load any blocks for new project
//             setHasInitialized(true);
//             setLoadingStatus("ready");
//           }
//           return;
...
//         // Priority 3: If no externalParentId and no forceNewProject, check for existing
//         // This handles the case of refreshing the page while working on a project
//         if (!externalParentId && !forceNewProject && !isNewProject) {
//           console.log("Looking for existing project...");
...
//       initializeParent();
//     }, [verifyParentExists, createParent, parentId, dispatch, isLoaded, externalParentId, loadBlocksForParent, forceNewProject, isNewProject, hasInitialized]);
...
//     // Load blocks when parent is ready and not already loaded
//     useEffect(() => {
//       if (isParentReady && parentIdRef.current && !isLoaded && !isForcedNew) {
//         console.log("Parent is ready, loading blocks for:", parentIdRef.current);
//         loadBlocksForParent(parentIdRef.current);
//       }
//     }, [isParentReady, isLoaded, loadBlocksForParent, isForcedNew]);
...
//               }
//               layoutType={currentBlockData?.layoutType || block.settings?.layoutType || "left-panel"}
//               backgroundImage={currentBlockData?.backgroundImage || block.settings?.backgroundImage || `${API_URL}/uploads/1766829715438.jpg`}
//               backgroundColor={currentBlockData?.backgroundColor || block.settings?.backgroundColor || "#2d5000"}
//               textColor={currentBlockData?.textColor || block.settings?.textColor || "#CFCFCF"}
//               backgroundFilter={block.settings?.backgroundFilter}
//               textAlign={(currentBlockData?.styles?.textAlign) || block.settings?.textAlign || "left"}
...
//               }
//               layoutType={currentBlockData?.layoutType || block.settings?.layoutType || "right-panel"}
//               backgroundImage={currentBlockData?.backgroundImage || block.settings?.backgroundImage || `${API_URL}/uploads/1766830308568.jpg`}
//               backgroundColor={currentBlockData?.backgroundColor || block.settings?.backgroundColor || "#c0b462"}
//               textColor={currentBlockData?.textColor || block.settings?.textColor || "#CFCFCF"}
//               backgroundFilter={block.settings?.backgroundFilter}
//               textAlign={(currentBlockData?.styles?.textAlign) || block.settings?.textAlign || "left"}
//               initialTitle={currentBlockData?.title || block.settings?.title || "Sales Proposal"}
...
//               }
//               layoutType={currentBlockData?.layoutType || block.settings?.layoutType || "top-panel"}
//               backgroundImage={currentBlockData?.backgroundImage || block.settings?.backgroundImage || `${API_URL}/uploads/1766830493922.avif`}
//               backgroundColor={currentBlockData?.backgroundColor || block.settings?.backgroundColor || "#2d5000"}
//               textColor={currentBlockData?.textColor || block.settings?.textColor || "#CFCFCF"}
//               backgroundFilter={block.settings?.backgroundFilter}
//               textAlign={(currentBlockData?.styles?.textAlign) || block.settings?.textAlign || "left"}
...
//               }
//               layoutType={currentBlockData?.layoutType || block.settings?.layoutType || "grid"}
//               backgroundImage={currentBlockData?.backgroundImage || block.settings?.backgroundImage || `${API_URL}/uploads/1766832158465.avif`}
//               backgroundColor={currentBlockData?.backgroundColor || block.settings?.backgroundColor || "#2d5000"}
//               textColor={currentBlockData?.textColor || block.settings?.textColor || "#CFCFCF"}
//               backgroundFilter={block.settings?.backgroundFilter}
//               textAlign={(currentBlockData?.styles?.textAlign) || block.settings?.textAlign || "left"}
...
const Canvas = forwardRef(
  (
    ...
    // New ref to track if initialization has already run (replaces state to avoid re-triggers)
    ...
    // Initialize parent on mount (runs only once due to ref guard)
    useEffect(() => {
      const initializeParent = async () => {
        ...
        // Priority 2: Use externalParentId if provided
        if (externalParentId) {
          console.log("Editing project with externalParentId:", externalParentId);
        ...
        // Priority 3: Check for existing project (refresh case)
        if (!externalParentId && !forceNewProject && !isNewProject) {
          console.log("Looking for existing project...");
