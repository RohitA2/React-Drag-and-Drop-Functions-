...
import React, { useState } from "react";
import { X, Copy, ExternalLink, Calendar, Edit, Save, X as CloseIcon, Plus, Clock, MapPin, MessageCircle } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useSelector } from "react-redux";
import { selectedUserId } from "../store/authSlice";
import ChatSection from "./ChatSection";
import { Button, OverlayTrigger, Tooltip, Form, Modal, Badge } from "react-bootstrap";
import { toast } from "react-toastify";
...
const ProjectDetails = ({ project, onClose }) => {
  ...
  try {
    ...
    // For creating schedule - send as array to match backend expectation
    const body = isCreatingSchedule
      ? {
          schedules: [{
            parentId: project.parentId || project.id,
    ...
    if (data.success) {
      if (isCreatingSchedule) {
        // Add new schedule to the project data
        const newSchedule = {
          id: data.id[0], // Backend returns array of IDs
          ...scheduleFormData,
          time: scheduleFormData.time + ':00'
        };
        project.schedules = [...(project.schedules || []), newSchedule];
  ...
  return (
    <div className="project-details-sidebar">
      <div className="project-details-header">
        ...
        <div className="project-meta">
          <div className="meta-item">
            <Calendar size={16} />
            <span>Created {new Date(project.createdAt || new Date()).toLocaleDateString()}</span>
          </div>
          <div className="meta-item">
            <Clock size={16} />
            <span>Expires {new Date(project.expirationDate || new Date()).toLocaleDateString()}</span>
          </div>
          {project.link && (
            <div className="meta-item">
          ...
          <div className="details-section">
            ...
            {project.schedules && project.schedules.length > 0 ? (
              ...
              <div className="no-schedules">
                ...
                <p className="text-muted mb-3">No schedules found</p>
                <Button
                  variant="outline-primary"
                  size="sm"
                  onClick={handleCreateSchedule}
                ...
                project.recipients.map((recipient, index) => (
                  ...
                  >
                    <div className="recipient-avatar">
                      {recipient.recipientName?.charAt(0).toUpperCase() || 'U'}
                    </div>
                    <div className="recipient-info">
                      <div className="recipient-main">
                        <p className="recipient-name">{recipient.recipientName}</p>
                        <Badge 
                          bg={getStatusVariant(recipient.status)} 
                      ...
                      <div className="recipient-details">
                        <div className="contact-item">
                          ...
                          <span className="contact-value">{recipient.recipientEmail}</span>
                          <OverlayTrigger
                            placement="top"
                            overlay={<Tooltip>Copy email</Tooltip>}
                          >
                            <Button
                              variant="link"
                              size="sm"
                        ...
                        <div className="contact-item">
                          ...
                          <span className="contact-value">
                            {recipient.recipientDetails?.phone || "Not provided"}
                          </span>
                          {recipient.recipientDetails?.phone && (
                            <OverlayTrigger
                              placement="top"
                              overlay={<Tooltip>Copy phone</Tooltip>}
                            >
                              <Button
                                variant="link"
                                size="sm"
      ...
      <Modal show={showScheduleModal} onHide={handleCloseScheduleModal} centered>
        ...
        <Modal.Footer>
          <Button variant="outline-secondary" onClick={handleCloseScheduleModal}>
            <CloseIcon size={14} className="me-1" /> Cancel
          </Button>
          <Button
            variant="primary"
      ...
      <style>{`
        ...
        .project-details-header {
          background: linear-gradient(135deg, #4364EE 0%, #3A56D4 100%);
          color: white;
          padding: 32px;
          position: relative;
          overflow: hidden;
        ...
        .schedule-item::before {
          ...
          right: 0;
          height: 1px;
          background: linear-gradient(90deg, transparent, rgba(67, 100, 238, 0.3), transparent);
        }

        .schedule-item:hover {
          transform: translateX(4px);
          box-shadow: 0 8px 25px rgba(67, 100, 238, 0.15);
        }
        ...
        .recipient-item.selected {
          border-color: #4364EE;
          background: rgba(67, 100, 238, 0.05);
        }
        ...
        .recipient-avatar {
          ...
          height: 48px;
          border-radius: 50%;
          background: linear-gradient(135deg, #4364EE, #3A56D4);
          display: flex;
          align-items: center;
          justify-content: center;
          color: white;
        ...
        .copy-btn:hover {
          color: #4364EE;
          background: rgba(67, 100, 238, 0.1);
        }
